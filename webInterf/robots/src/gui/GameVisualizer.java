...
public class GameVisualizer extends JPanel
{
    ...
    public String backAnswer(String message){

        String[] mess_arr = message.split(";");
        try {
            m_targetPositionX = Integer.parseInt(mess_arr[0]);
            m_targetPositionY = Integer.parseInt(mess_arr[1]);
            m_robotPositionX = Double.parseDouble(mess_arr[2]);
            m_robotPositionY = Double.parseDouble(mess_arr[3]);
            m_robotDirection = Double.parseDouble(mess_arr[4]);

            if (distance(m_robotPositionX, m_robotPositionY, m_targetPositionX, m_targetPositionY) < 1)
            {
                return Math.round(m_robotPositionX) + ";" + Math.round(m_robotPositionY) + ";" + Math.round(m_robotDirection);
            }
            onModelUpdateEvent();
        }
        catch (java.lang.NumberFormatException e)
        {
            ...
            return m_robotPositionX + ";" + m_robotPositionY + ";" + m_robotDirection;
        }
    }
